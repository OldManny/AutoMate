...
def log_operation(operation, master_file, backup_info):
    ...
    """
    if not LOG_FILE:
        print("ERROR: LOG_FILE path not set in undo_manager module.")
        return
...
def undo_file_operation():
    ...
    """
    if not LOG_FILE:
        raise ValueError("LOG_FILE path not set")
    ...
    try:
        ...
        # Remove log file at the end
        os.remove(LOG_FILE)
    except FileNotFoundError:
        raise ValueError("Operation log file missing.")
    except Exception as e:
        print(f"Error during file undo: {e}")
        raise
...
def undo_data_operation():
    ...
    try:
        ...
        print(f"Error during data undo: {e}")
        raise
...
def clear_previous_log():
    ...
    try:
        ...
        pass  # Log file already gone, nothing to do
    except Exception as e:
        print(f"Error clearing previous log/backups: {e}")
